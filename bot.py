...
async def process_message(chatid, message, bot, userid, is_group):
    ...
    logging.info("===============================================================")
    logging.info(
        f'Message from [{known_users[userid]}] - "{message}" Group? [{str(is_group)}]'
    )
    if validators.url(message):
        downloading_msg_details = await bot.send_message(
            chat_id=chatid, text="downloading..."
        )
        ...
        await bot.delete_message(
            chat_id=chatid, message_id=downloading_msg_details.message_id
        )
    else:
...
async def save_audio_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        update.message.audio.file_name + " was sent to Tipu to be saved and replicated"
    )
    ...
    await context.bot.send_message(
        ...
        text=update.message.audio.file_name
        + " was saved to disk. Replication will follow.",
    )
...
def start_pcloud_upload():
    ...
    _thread.start_new_thread(
        os.system, ('python3 "' + config.PROJECT_DIR + '/pcloudUpload.py"',)
    )
